 ...
 * MTCNN 人脸探测模型
 */
public class MTCNN implements DetectModel {

    private static final String TAG = MTCNN.class.getSimpleName();
    // 模型资源
    private static final int MODEL_IDENTIFIER = R.raw.mtcnn_20180726;
    ...
    private boolean loadModel() {
        ...
        try {
            ...
            Log.d(TAG, "MTCNN load model success");
        } catch (Exception e) {
            Log.e(TAG, "MTCNN load model failed" + e);
            return false;
        }
        return true;
    }
     ...
     */
    private void nms(Vector<Face> boxes, float threshold, String method) {
        //NMS.两两比对
        //int delete_cnt=0;
        int cnt = 0;
        for (int i = 0; i < boxes.size(); i++) {
            Face box = boxes.get(i);
            if (!box.deleted) {
                //score<0表示当前矩形框被删除
                for (int j = i + 1; j < boxes.size(); j++) {
                    Face box2 = boxes.get(j);
                    if (!box2.deleted) {
    ...
    private int generateBoxes(float[][] prob, float[][][] bias, float scale, float threshold, Vector<Face> boxes) {
        int h = prob.length;
        int w = prob[0].length;
    ...
    private Vector<Face> PNet(Bitmap bitmap, int minSize) {
        ...
        Vector<Face> totalBoxes = new Vector<Face>();
        ...
        while (currentFaceSize <= whMin) {
            ...
            //(3)数据解析
            Vector<Face> curBoxes = new Vector<Face>();
            generateBoxes(PNetOutProb, PNetOutBias, scale, PNetThreshold, curBoxes);
            //Log.i(TAG,"[*]CNN Output Face number:"+curBoxes.size()+" Scale:"+scale);
            //(4)nms 0.5
    ...
    private void crop_and_resize(Bitmap bitmap, Face box, int size, float[] data) {
        //(2)crop and resize
        Matrix matrix = new Matrix();
     ...
     */
    private void RNetForward(float[] RNetIn, Vector<Face> boxes) {
     ...
     * Refine Net
     */
    private Vector<Face> RNet(Bitmap bitmap, Vector<Face> boxes) {
        //RNet Input Init
        int num = boxes.size();
     ...
     */
    private void ONetForward(float[] ONetIn, Vector<Face> boxes) {
     ...
     * ONet
     */
    private Vector<Face> ONet(Bitmap bitmap, Vector<Face> boxes) {
        //ONet Input Init
        int num = boxes.size();
     ...
     */
    @Override
    public DetectedFaces detectFaces(Bitmap detectBitmap, int detectMiniFaceSize) {
        long t_start = System.currentTimeMillis();
        //【1】PNet generate candidate boxes
        Vector<Face> faces = PNet(detectBitmap, detectMiniFaceSize);
        square_limit(faces, detectBitmap.getWidth(), detectBitmap.getHeight());
        //【2】RNet
        faces = RNet(detectBitmap, faces);
        square_limit(faces, detectBitmap.getWidth(), detectBitmap.getHeight());
        //【3】ONet
        faces = ONet(detectBitmap, faces);
        // 结构转换
        ArrayList<Face> listFaces = new ArrayList(faces);
        DetectedFaces detectedFaces = new DetectedFaces(detectBitmap, listFaces);
