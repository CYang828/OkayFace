...
public class Face {
    ...
    public Rect transform2OriginalBox(float scaleX, float scaleY) {
        Rect rect = new Rect();
        rect.left = Math.round(box[0] * scaleX);
        rect.top = Math.round(box[1] * scaleY);
        rect.right = Math.round(box[2] * scaleX);
        rect.bottom = Math.round(box[3] * scaleY);
        Log.i("Original Rect:", rect.toString());
        originalBox = rect;
        return rect;
    }
