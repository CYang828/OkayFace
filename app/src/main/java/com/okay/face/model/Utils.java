...
public class Utils {
    ...
    //box转化为rect
    public static Rect[] boxes2rects(Vector<Face> boxes){
        int cnt=0;
    ...
    //删除做了delete标记的box
    public static Vector<Face> updateBoxes(Vector<Face> boxes){
        Vector<Face> b=new Vector<Face>();
        for (int i=0;i<boxes.size();i++)
            if (!boxes.get(i).deleted)
